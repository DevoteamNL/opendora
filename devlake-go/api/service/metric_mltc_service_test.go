...
func TestMetricMltcServiceMltcService_ServeRequest(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			params:         ServiceParameters{TypeQuery: "mltc", Aggregation: "weekly", Project: "", To: 0, From: 0},
		...
		{
			name:           "should return an error with an unexpected error from the database",
			params:         ServiceParameters{TypeQuery: "mltc", Aggregation: "monthly", Project: "", To: 0, From: 0},
			mockClient:     sql_client.MockClient{MockDeploymentsDataMap: errorMockMap},
		...
		{
			name:           "should return an error with an unexpected error from the database",
			params:         ServiceParameters{TypeQuery: "mltc", Aggregation: "quarterly", Project: "", To: 0, From: 0},
			mockClient:     sql_client.MockClient{MockDeploymentsDataMap: errorMockMap},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mltcService := MetricMltcService{Client: tt.mockClient}
			got, err := mltcService.ServeRequest(tt.params)
			...
			if !reflect.DeepEqual(got, tt.expectResponse) {
				t.Errorf("MltcService.ServeRequest() = %v, want %v", got, tt.expectResponse)
			}
		})
	}
}
